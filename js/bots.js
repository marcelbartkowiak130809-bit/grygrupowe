...
export const BOT_DIFFICULTIES = [
  { id: "easy", label: "Łatwy", accuracy: 0.56, minDelay: 1800, maxDelay: 5200 },
  { id: "normal", label: "Normalny", accuracy: 0.72, minDelay: 1100, maxDelay: 3600 },
  { id: "hard", label: "Trudny", accuracy: 0.86, minDelay: 700, maxDelay: 2400 },
  { id: "expert", label: "Ekspert", accuracy: 0.95, minDelay: 420, maxDelay: 1500 },
];
...
export function botDifficulty(room) {
  return BOT_DIFFICULTIES.find(item => item.id === room?.settings?.botDifficulty) || BOT_DIFFICULTIES[1];
}
export function botDelay(room, kind = "answer") {
  const level = botDifficulty(room), span = level.maxDelay - level.minDelay;
...
export function botShouldBeCorrect(room) { return Math.random() < botDifficulty(room).accuracy; }
...
export function roomAllowsBots(room, mode) {
  return Boolean(room?.isPrivate && room.status === "lobby" && mode?.supportsLobby && !["co-wolisz", "ranking"].includes(mode.id) && !(mode.id === "quiz" && room.settings?.quizVariant === "competitive"));
}

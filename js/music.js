...
function playerRows(game, accounts, field = "scores", winners = []) {
  const winnerSet = new Set(winners);
  return [...array(game.players)].sort((a, b) => Number(game[field]?.[b] || 0) - Number(game[field]?.[a] || 0)).map((uid, index) => `<div class="music-ranking-row"><div class="music-ranking-player"><i>${index + 1}</i>${resultPlayerMiniHtml(accounts[uid], winnerSet.has(uid) ? "win" : "lose")}</div><b>${Number(game[field]?.[uid] || 0)} pkt</b></div>`).join("");
}
...
export function renderMusicDuelGame(root, { room, accounts, currentUser }, actions) {
  ...
  if (game.phase === "selecting") {
    ...
    content += `<h2>Wynik rundy</h2>${playlistHtml(game.roundResult?.submissions || game.submissions, accounts, { namespace })}<div class="music-result-list">${Object.entries(game.roundResult?.submissions || {}).map(([uid, track]) => track ? `<div><span>${escapeHtml(nick(accounts, uid))}</span><b>${Number(game.roundResult?.voteCounts?.[uid] || 0)} gł.</b></div>` : "").join("")}</div><p class="music-winner">${winners.length ? `🏆 ${winners.map(uid => escapeHtml(nick(accounts, uid))).join(", ")} wygrywa rundę!` : "Nikt nie zdobył punktu."}</p><div class="music-ranking">${playerRows(game, accounts, "scores", winners)}</div><button id="music-duel-next" class="primary">${Number(game.round) >= Number(game.totalRounds) ? "Pokaż podsumowanie" : "Następna runda"}</button>`;
  } else {
    ...
    content += `<div class="music-final"><span>🏆</span><h2>Koniec pojedynku</h2><p>${winners.length ? `Wygrywa ${winners.map(uid => escapeHtml(nick(accounts, uid))).join(", ")}!` : "Tym razem nie było zwycięzcy."}</p><div class="music-ranking">${playerRows(game, accounts, "scores", winners)}</div></div><button id="music-duel-lobby" class="primary">Zagraj ponownie</button>`;
  }
...
export function renderMusicArenaGame(root, { room, accounts, currentUser }, actions) {
  ...
  if (game.phase === "selecting") {
    ...
    const winners = game.roundResult?.winners || [];
    content += `<h2>Wynik bitwy</h2>${playlistHtml(game.roundResult?.submissions || game.submissions, accounts, { arena:true, namespace })}<div class="music-result-list">${Object.entries(game.roundResult?.submissions || {}).map(([uid, track]) => track ? `<div><span>${escapeHtml(nick(accounts, uid))}</span><b>${Number(game.roundResult?.voteCounts?.[uid] || 0)} gł.</b></div>` : "").join("")}</div><p class="music-winner">${winners.length ? `🏆 Wygrywa ${winners.map(uid => escapeHtml(nick(accounts, uid))).join(", ")}!` : "Remis — nikt nie zdobywa punktu."}</p><div class="music-ranking">${playerRows(game, accounts, "wins", winners)}</div><button id="music-arena-next" class="primary">${Number(game.round) >= Number(game.totalRounds) ? "Pokaż podsumowanie" : "Następna bitwa"}</button>`;
  } else {
    const top = Math.max(0, ...Object.values(game.scores || {}).map(Number)), winners = game.players.filter(uid => Number(game.scores?.[uid] || 0) === top && top > 0);
    content += `<div class="music-final"><span>🏆</span><h2>Koniec bitwy</h2><p>${winners.length ? `Najwięcej zwycięstw ma ${winners.map(uid => escapeHtml(nick(accounts, uid))).join(", ")}!` : "Nie wyłoniono zwycięzcy."}</p><div class="music-ranking">${playerRows(game, accounts, "wins", winners)}</div></div><button id="music-arena-lobby" class="primary">Zagraj ponownie</button>`;
  }

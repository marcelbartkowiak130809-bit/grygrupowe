...
import { inGamePurchaseById } from "./gamePasses.js?v=20260831-2";
...
export function createNumberMysteryGame(players, settings = {}) {
  ...
  return {
    ...
    phaseEndsAt: phaseEnd(questionTime), winner: "", finished: false, privateHints:{}, purchaseUses:{},
  };
}
...
export function renderNumberMysteryGame(root, { room, accounts, currentUser }, actions) {
  ...
  const hintPanel=hint ? `<p class="number-mystery-private-hint">🔢 Twoja wskazówka: ${escapeHtml(hint)}</p>` : room.settings?.gamePurchases !== false && !hintUsed ? `<button class="ghost number-mystery-hint-button" data-number-mystery-hint>🔢 Wskazówka za ${hintPurchase?.price || 500}$</button>` : "";

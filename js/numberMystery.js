...
export function renderNumberMysteryGame(root, { room, accounts, currentUser }, actions) {
  ...
  root.innerHTML = `<main class="page number-mystery-page enter"><section class="panel number-mystery-panel"><p class="eyebrow">TAJEMNICZA LICZBA · ${game.roundMode === "rounds" ? `RUNDA ${Math.min(game.round, game.rounds)}/${game.rounds}` : "POLOWANIE"}</p><h1>Zgadnij swój numer</h1><p class="muted">Twój numer jest ukryty. Znasz numer przeciwnika: <strong>${game.numbers?.[opponent] ?? "—"}</strong>.</p><div class="number-mystery-status"><span>${game.phase === "guess" ? "Czas na końcowe typowanie" : myTurn ? "Twoja kolej — możesz zadać pytanie albo zgadnąć" : mustAnswer ? "Odpowiedz na pytanie" : "Czekamy na ruch przeciwnika"}</span><b data-number-mystery-countdown></b></div>${composer}${guessAllowed ? `<form id="number-mystery-guess" class="number-mystery-guess"><label>Zgadnij zamiast pytania<input id="number-mystery-guess-input" type="number" min="1" max="150" placeholder="Wpisz liczbę od 1 do 150"><button class="primary">Zgaduję</button></label><small>Jeśli nie trafisz, ta tura przepada i kolej przechodzi na przeciwnika.</small></form>` : ""}<section class="number-mystery-history"><div class="section-intro"><div><p class="eyebrow">HISTORIA PYTAŃ</p><h2>Co już padło?</h2></div><span class="badge">${game.history?.length || 0}</span></div><ol>${history}</ol></section></section></main>`;

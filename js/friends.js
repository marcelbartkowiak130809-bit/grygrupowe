...
function playerStatus(uid, rooms = [], presence = {}) {
  if (!presence[uid]) return { label:"offline", detail:"Offline" };
  ...
  return { label:"online", detail:"Dostępny" };
}
...
export function friendsModal(context) {
  ...
  let tab = context.inviteMode ? "friends" : "friends", searchResults = [], lastViewSignature = "";
  ...
  const render = () => {
    ...
    const viewSignature = JSON.stringify({ tab, searchResults:searchResults.map(item => item.uid), friends, incoming:incoming.map(item => `${item.id}:${item.status||""}`), outgoing:outgoing.map(item => `${item.id}:${item.status||""}`), rooms:current.rooms.filter(room => room.players?.some(uid => friends.includes(uid))).map(room => `${room.roomId}:${room.status}:${room.players?.length}`), presence:friends.map(uid => `${uid}:${Boolean(current.presence?.[uid])}`) });
    if (viewSignature === lastViewSignature && modal.querySelector("[data-friends-close]")) return;

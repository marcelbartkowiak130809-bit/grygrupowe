...
function playerStatus(uid, rooms = [], presence = {}) {
  ...
  if (room?.status === "playing") return { label:"🎮 w grze", detail:`Gra: ${room.name || "Gra"}` };
  if (room?.status === "lobby") { const mode = room.gameMode || "gra"; return { label:"online", detail:`Lobby: ${room.players.length}/${room.maxPlayers || 8} · ${mode}` }; }
...
function friendRow(uid, item, context, actionHtml = "") {
  ...
  const statusClass = status.label === "offline" ? "offline" : status.label.startsWith("🎮") ? "playing" : "online";
...
export function friendsModal(context) {
  ...
  const getContext = () => ({ ...context, account:context.actions.getAccount(), directory:context.actions.getFriendDirectory(), rooms:context.actions.getRooms() });
  const render = () => {
    const current = getContext(), account = current.account, friends = (account?.friends || []).filter(uid => current.directory[uid]);
    const incoming = Object.values(incomingRequests(account)); const outgoing = Object.values(outgoingRequests(account));
    ...
    const content = tab === "friends" ? `<div class="friend-search"><input id="friend-search-input" placeholder="Wyszukaj gracza po nicku" maxlength="18"><button class="primary" id="friend-search">Szukaj</button></div><div class="friend-search-results">${searchResults.map(item => friendRow(item.uid,item,current,isFriend(account,item.uid) ? "" : `<button class="primary" data-friend-add="${item.uid}">Dodaj</button>`)).join("")}</div><div class="friends-list">${friendRows}</div>` : tab === "incoming" ? `<div class="friends-list">${incoming.length ? incoming.map(request => requestRow(request,current,context.actions)).join("") : `<p class="muted">Brak otrzymanych zaproszeń.</p>`}</div>` : `<div class="friends-list">${outgoing.length ? outgoing.map(request => `<article class="friend-request-row"><div><b>${escapeHtml(request.toNick || "Gracz")}</b><small>Zaproszenie oczekuje na odpowiedź.</small></div><button class="ghost" data-friend-cancel="${request.id}">Anuluj</button></article>`).join("") : `<p class="muted">Brak wysłanych zaproszeń.</p>`}</div>`;
    modal.innerHTML = `<section class="modal friends-modal enter" role="dialog" aria-modal="true"><div class="modal-title"><div><p class="eyebrow">SPOŁECZNOŚĆ</p><h2>Znajomi</h2></div><button class="icon-btn" data-friends-close>${icon("x",18)}</button></div><div class="friends-tabs"><button class="${tab === "friends" ? "active" : ""}" data-friends-tab="friends">Moi znajomi</button><button class="${tab === "incoming" ? "active" : ""}" data-friends-tab="incoming">Otrzymane ${incoming.length ? `<b>${incoming.length}</b>` : ""}</button><button class="${tab === "outgoing" ? "active" : ""}" data-friends-tab="outgoing">Wysłane</button></div>${context.inviteMode ? `<p class="friend-invite-note">Wybierz znajomego, którego chcesz zaprosić do lobby.</p>` : ""}${content}</section>`;
    modal.querySelector("[data-friends-close]").addEventListener("click", () => context.actions.closeModal(modal));
    ...
    modal.querySelectorAll("[data-friend-add]").forEach(button => button.addEventListener("click", async () => { await context.actions.sendFriendRequest(button.dataset.friendAdd); render(); }));
    ...
    modal.querySelectorAll("[data-friend-accept]").forEach(button => button.addEventListener("click", async () => { await context.actions.acceptFriendRequest(button.dataset.friendAccept); render(); }));
    modal.querySelectorAll("[data-friend-reject]").forEach(button => button.addEventListener("click", async () => { await context.actions.rejectFriendRequest(button.dataset.friendReject); render(); }));
    modal.querySelectorAll("[data-friend-ignore]").forEach(button => button.addEventListener("click", async () => { await context.actions.rejectFriendRequest(button.dataset.friendIgnore); render(); }));
    ...
    modal.querySelectorAll("[data-friend-cancel]").forEach(button => button.addEventListener("click", async () => { await context.actions.cancelFriendRequest(button.dataset.friendCancel); render(); }));
  };
  render(); document.body.append(modal); context.actions.playSound?.("modalOpen"); return modal;
}
...
export function showFriendNotification(request, context) {
  ...
  toast.querySelector("[data-toast-accept]")?.addEventListener("click",()=>context.actions.acceptFriendRequest(request.id).then(()=>toast.remove()));
  toast.querySelector("[data-toast-join]")?.addEventListener("click",()=>context.actions.acceptGameInvite(request).then(()=>toast.remove()));
  toast.querySelector("[data-toast-join-request]")?.addEventListener("click",()=>context.actions.acceptJoinRequest(request.id).then(()=>toast.remove()));
  toast.querySelector("[data-toast-dismiss]").addEventListener("click",()=>{context.actions.rejectFriendRequest(request.id);toast.remove();});

...
export function renderFriendshipTestGame(root, { room, accounts, currentUser }, actions) {
  stopFriendshipTimer(); const g = room.game, s = { ...friendshipDefaults, ...room.settings }; let body;
  ...
  else if (g.phase === "roundSummary") body = `<section class="panel center phase-card"><p class="eyebrow">PODSUMOWANIE RUNDY</p><h1>Kto zna ekipę najlepiej?</h1><div class="result-player-grid">${room.players.map(uid => `<article>${mini(accounts[uid])}<strong>+${g.roundScores[uid] || 0} pkt</strong></article>`).join("")}</div><button class="primary" id="friend-round-next">${g.round >= s.rounds ? "Zobacz tabelę" : "Następna runda"}</button></section>`;
  else { const ranking = Object.entries(g.scores).sort((a, b) => b[1] - a[1]); body = `<section class="panel center phase-card"><p class="eyebrow">KONIEC TESTU</p><h1>Tabela znajomości</h1><div class="final-ranking">${ranking.map(([uid, score], index) => `<article><b>#${index + 1}</b>${mini(accounts[uid])}<strong>${score} trafień</strong></article>`).join("")}</div>${s.rewardCoins ? '<p class="money-pop">Każde trafienie to +25$.</p>' : ""}<button class="primary" id="friend-lobby">Wróć do lobby</button></section>`; }

...
import { $, avatarHtml, escapeHtml, normalizeAnswer, playerMiniHtml, resultPlayerMiniHtml, safeCosmeticClass } from "./utils.js?v=20260903-7";
...
const settingsWithDefaults = settings => {
  ...
  s.rounds = clampNumber(s.rounds, 1, 20, identityDefaults.rounds);
...
<label>Liczba rund<select data-identity-setting="rounds" ${isHost ? "" : "disabled"}>${Array.from({ length:20 }, (_, index) => index + 1).map(n => `<option value="${n}" ${s.rounds === n ? "selected" : ""}>${n}</option>`).join("")}</select></label>
...
export function renderIdentityGame(root, { room, accounts, currentUser }, actions) {
  ...
  const main = g.phase === "results"
    ? `<section class="panel center identity-results"><h1>Koniec gry</h1><div class="final-ranking">${Object.entries(g.scores).sort((a,b) => b[1] - a[1]).map(([uid,n], index, rows) => { const topScore = Math.max(0, ...rows.map(([,score]) => Number(score) || 0)); return `<article><b>#${index + 1}</b>${resultPlayerMiniHtml(accounts[uid], Number(n) === topScore && topScore > 0 ? "win" : "lose")}<strong>${n} pkt</strong></article>`; }).join("")}</div><h2>Kto kim byl</h2>${resultsHistory(g, accounts)}<button class="primary" id="identity-again">Wroc do lobby</button></section>`
    : g.phase === "extendVote"

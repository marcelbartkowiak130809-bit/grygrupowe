import { $, avatarHtml, escapeHtml, icon } from "./utils.js";
...
export function accountModal(profile, actions) {
  ...
  backdrop.innerHTML=`<section class="modal account-modal enter" role="dialog" aria-modal="true" aria-labelledby="account-title">
    ...
    ${profile.nickOnly ? "" : `<section class="account-info-row"><div><span class="muted">Data urodzenia</span><b>${escapeHtml(profile.birthDate || "nie podano")}</b><p class="tiny">Nie można zmienić jej samodzielnie. Wniosek trafia do administracji.</p></div><button class="ghost" id="birth-change-request">Zmień</button></section>`}
    ...
    <div class="account-actions">${profile.nickOnly?'<button class="primary" id="upgrade-account">Zaloguj się na konto</button>':`<button class="ghost" id="open-inbox">Inbox${inboxCount ? ` (${inboxCount})` : ""}</button>${isAdmin?'<button class="primary" id="open-admin-panel">Panel admina</button>':""}`}<button class="danger" id="account-logout">Wyloguj</button></div>
  </section>`;
  const close=()=>actions.closeModal(backdrop);backdrop.querySelector("[data-close]").addEventListener("click",close);

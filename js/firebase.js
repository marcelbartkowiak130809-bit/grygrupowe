...
export async function syncRoomState(room) {
  ...
  try {
    ...
    const payload = {
      roomId:room.roomId, gameMode:room.gameMode, maxPlayers:room.maxPlayers == null ? null : Math.max(1, Number(room.maxPlayers) || 1), name:room.name || "Pokój dla ekipy", isPrivate:Boolean(room.isPrivate), passwordHash:room.passwordHash || "", hostUid:room.hostUid || null,

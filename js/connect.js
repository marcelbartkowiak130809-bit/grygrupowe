...
function rankingHtml(game, accounts) {
  return [...(game.players || [])].sort((a, b) => Number(game.scores?.[b] || 0) - Number(game.scores?.[a] || 0)).map((uid, index) => `<div class="connect-ranking-row"><span>${index + 1}. ${escapeHtml(nick(accounts, uid))}</span><b>${Number(game.scores?.[uid] || 0)} pkt</b></div>`).join("");
}

...
export function renderShop(root, { profile }, actions) {
  ...
  root.innerHTML = `<main class="page shop-page enter">
    ...
    <section class="panel owned-cosmetics-panel"><div class="section-heading"><div><p class="eyebrow">GARDEROBA</p><h2>Twoje kosmetyki</h2></div><span class="badge">${cosmetics.filter(item => profile.ownedCosmetics[item.id]).length}</span></div><p class="muted">Każdy efekt zobaczysz dokładnie tak, jak wygląda przy graczu. Kliknij kartę, aby go założyć.</p>
      <div class="cosmetic-list">${cosmetics.filter(item => profile.ownedCosmetics[item.id]).map(item => {

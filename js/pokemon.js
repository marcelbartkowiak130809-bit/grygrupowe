...
export function renderPokemonGame(root, { room, accounts, currentUser }, actions) {
  ...
  if (game.mode === "pokemon-last-letter") { game.order = Array.isArray(game.order) && game.order.length ? game.order : [currentUser]; game.chain = Array.isArray(game.chain) ? game.chain : []; game.usedIds = Array.isArray(game.usedIds) ? game.usedIds : []; game.turnIndex = Math.max(0, Math.min(Number(game.turnIndex) || 0, game.order.length - 1)); }
  ...
  let body = "";
  if (game.phase === "result") { root.innerHTML = `<main class="page pokemon-game-page enter">${game.mode === "pokemon-dex" ? dexResultMarkup(game, accounts) : resultMarkup(game, accounts, actions, game.mode === "pokemon-auction" ? "Wynik aukcji" : "Wyniki rundy")}</main>`; root.querySelector("#pokemon-next")?.addEventListener("click", actions.pokemonNextRound); return; }
  if (game.mode === "pokemon-dex") { const target = pokemonDex.find(item => item.id === game.target), answer=game.answers?.[currentUser]; body = `<p class="eyebrow">NAJBLIŻSZY NUMER POKEDEX · RUNDA ${game.round}/${settings.rounds || 5}</p><h1>Znajdź Pokémona najbliżej numeru</h1><div class="pokemon-target">#${target.id}</div><p class="muted">Wpisz nazwę, ale nie podawaj numeru Dex.</p>${timerMarkup(game)}${dexFormMarkup(answer)}`; }
...
export function renderPokemonLobbySettings(room, isHost) {
  ...
  const common = `<label>Czas odpowiedzi <input data-pokemon-setting="answerTime" type="number" min="5" max="60" value="${s.answerTime || 15}" ${isHost ? "" : "disabled"}></label>${room.gameMode === "pokemon-types" ? `<label>Czas wyboru typu <input data-pokemon-setting="selectTime" type="number" min="5" max="30" value="${s.selectTime || 10}" ${isHost ? "" : "disabled"}></label>` : ""}<label>Liczba rund <input data-pokemon-setting="rounds" type="number" min="1" max="20" value="${s.rounds || 5}" ${isHost ? "" : "disabled"}></label>${room.gameMode === "pokemon-evolution" ? `<label>Dozwolone kierunki<select data-pokemon-setting="evolutionMode" ${isHost ? "" : "disabled"}><option value="both" ${s.evolutionMode !== "later" ? "selected" : ""}>Wcześniejsze i późniejsze ewolucje</option><option value="later" ${s.evolutionMode === "later" ? "selected" : ""}>Tylko późniejsze ewolucje</option></select></label>` : ""}<div><p class="tiny">GENERACJE</p><div class="pokemon-types">${generations}</div></div>`;

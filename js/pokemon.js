...
function dexFormMarkup(answered) { return `<form class="pokemon-answer-form" data-pokemon-form="answer"><div class="pokemon-autocomplete"><input id="pokemon-answer" autocomplete="off" placeholder="Nazwa Pokémona" ${answered ? "disabled" : ""}><div id="pokemon-suggestions" class="pokemon-suggestions"></div></div><button class="primary" type="submit" ${answered ? "disabled" : ""}>${answered ? "Wysłano" : "Odpowiedz"}</button></form>`; }
...
export function renderPokemonGame(root, { room, accounts, currentUser }, actions) {
  ...
  if (game.mode === "pokemon-dex") { const target = pokemonDex.find(item => item.id === game.target); body = `<p class="eyebrow">NAJBLIŻSZY NUMER POKEDEX</p><h1>Znajdź Pokémona najbliżej numeru</h1><div class="pokemon-target">#${target.id}</div><p class="muted">Wpisz nazwę, ale nie podawaj numeru Dex.</p>${timerMarkup(game)}${dexFormMarkup(Boolean(game.answers?.[currentUser]))}`; }

...
function matchTypeCard(item) { return `<div class="pokemon-card">${sprite(item)}<div><b>${escapeHtml(pokemonDisplayName(item))}</b><small>#${item.id} · Gen ${item.generation}</small></div></div>`; }

import { pokemonDex } from "./pokemonData.js?v=20260804-1";
...
function resultMarkup(game, accounts, actions, title = "Wyniki rundy") { const rows = (game.ranking || []).map(row => `<div class="pokemon-result-row"><b>${escapeHtml(accounts[row.uid]?.nick || "Gracz")}</b><span>${row.answer ? escapeHtml(label(row.answer)) : row.bst != null ? `${row.bst} BST` : row.difference != null ? (Number.isFinite(row.difference) ? `różnica ${row.difference}` : "brak odpowiedzi") : row.correct ? "trafienie" : "-"}</span></div>`).join(""); return `<section class="panel pokemon-panel center"><p class="eyebrow">PODSUMOWANIE</p><h1>${title}</h1><div class="pokemon-results">${rows || "<p class=muted>Brak wyników.</p>"}</div><button class="primary" id="pokemon-next">Następna runda</button></section>`; }
function card(item, showBst = false) { return `<div class="pokemon-card"><img src="${item.sprite}" alt=""><div><b>${escapeHtml(item.name)}</b><small>#${item.id} · Gen ${item.generation} · ${item.types.map(type => typeNames[type] || type).join(" / ")}</small>${showBst ? `<strong>${item.bst} BST</strong>` : ""}</div></div>`; }

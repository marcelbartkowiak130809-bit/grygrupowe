...
import { avatarHtml, escapeHtml, icon } from "./utils.js?v=20260613-2";
...
export function renderPokemonGame(root, { room, accounts, currentUser }, actions) {
  ...
  let body = "";
  if (game.phase === "result") { root.innerHTML = `<main class="page pokemon-game-page enter">${game.mode === "pokemon-dex" ? dexResultMarkup(game, accounts) : game.mode === "pokemon-last-letter" ? lastLetterResultMarkup(game, accounts, settings) : resultMarkup(game, accounts, actions, game.mode === "pokemon-auction" ? "Wynik aukcji" : "Wyniki rundy")}</main>`; root.querySelector("#pokemon-next")?.addEventListener("click", actions.pokemonNextRound); root.querySelector("#pokemon-lobby")?.addEventListener("click", actions.returnToRoom); return; }
...
export function renderPokemonLobbySettings(room, isHost) {
  ...
  const roundOrHearts = room.gameMode === "pokemon-last-letter" ? `<label>Serca na gracza <input data-pokemon-setting="hearts" type="number" min="1" max="5" value="${Math.max(1, Math.min(5, s.hearts || 3))}" ${isHost ? "" : "disabled"}></label>` : `<label>Liczba rund <input data-pokemon-setting="rounds" type="number" min="1" max="20" value="${s.rounds || 5}" ${isHost ? "" : "disabled"}></label>`;

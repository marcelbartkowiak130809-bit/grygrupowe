...
function popularityRanking(game, accounts, room, winners = []) {
  ...
  return [...array(game.players)].sort((a, b) => Number(game.scores?.[b] || 0) - Number(game.scores?.[a] || 0)).map((uid, index) => `<div class="popularity-ranking-row"><div class="popularity-ranking-player"><i class="popularity-ranking-place">${index + 1}.</i>${resultPlayerMiniHtml(userProfile(accounts, room, uid), winnerSet.has(uid) ? "win" : "lose", "popularity-result-player")}</div><strong>${Number(game.scores?.[uid] || 0)} pkt</strong></div>`).join("");
}

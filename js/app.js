...
import { createNumberMysteryGame, NumberMysteryEngine, stopNumberMysteryTimer } from "./numberMystery.js?v=20260823-2";
...
import { isModeLocked, lockedModeMessage } from "./upcomingModes.js?v=20260823-2";
...
function settleAdditionalModeResult(room) {
  const supported = new Set(["marker", "sequence", "family", "word-chain", "number-mystery", "unique-answer"]);
  ...
  const coinBase = { marker:35, sequence:40, family:45, "word-chain":40, "number-mystery":45, "unique-answer":45 }[room.gameMode] || 35;
  const xpBase = { marker:35, sequence:40, family:45, "word-chain":40, "number-mystery":45, "unique-answer":45 }[room.gameMode] || 35;
...
function renderNow(options = {}) {
  ...
  lastRenderedScreenSignature=currentScreenSignature();
  stopShopTimer(); stopGameTimer(); stopImpostorTimer(); stopIdentityTimer(); stopOtherQuestionTimer(); stopMostLikelyTimer(); stopFriendshipTimer(); stopPoisonCandyTimer(); stopBombTimer(); stopFiveSecondsTimer(); stopClockTimer(); stopPokemonTimer(); stopWavelengthTimer(); stopQuizTimer(); stopMathematicsTimer(); stopFamilyTimer(); stopWordChainTimer(); stopSequenceTimer(); stopMarkerTimer(); stopNumberMysteryTimer(); stopUniqueAnswerTimer();
